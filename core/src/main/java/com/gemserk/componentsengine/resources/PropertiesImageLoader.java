		...
		try {
			...
			for (String imageKey : imagesMap.stringPropertyNames()) {
				...
				if (values.length == 1) {
					...
					// I have an alpha mask specified
					String imagePath = values[0];
					String alphaMaskPath = values[1];
					
					BufferedImage image = ImageIO.read(this.getClass().getClassLoader().getResourceAsStream(imagePath));
					...
					imageManager.addImage(imageKey, new Image(BufferedImageUtil.getTexture(resourceName, applyAlphaMask(image, alphaMask))));
				}
			}
		} catch (IOException e) {
